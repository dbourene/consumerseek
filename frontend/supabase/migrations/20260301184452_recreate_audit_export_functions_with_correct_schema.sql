/*
  # Recreate audit export functions with correct column mappings

  1. Problem
    - Existing functions reference non-existent columns
    - Need to drop and recreate with correct schema

  2. Solution
    - Drop old functions
    - Recreate with actual database columns
    - Match specification requirements from AUDIT_ENEDIS_GUIDE.md

  3. Tables
    - LOGS_CONSENTEMENT: Full consent registry with PRM granularity
    - LOGS_REVOCATION: Revocation events only
    - LOGS_APPELS_API: API call tracking and conformity
*/
...
-- Tab 1: LOGS_CONSENTEMENT (1 ligne = 1 PRM autorisé)
...
BEGIN
  ...
  WITH prm_agg AS (
    SELECT 
      autorisation_id,
      string_agg(prm_numero, ', ' ORDER BY created_at) as prm_list
    FROM prm
    GROUP BY autorisation_id
  )
  SELECT 
...
-- Tab 2: LOGS_REVOCATION (événements de révocation)
...
-- Tab 3: LOGS_APPELS_API (suivi des appels API et conformité)
