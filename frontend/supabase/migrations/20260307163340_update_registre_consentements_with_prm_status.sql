/*
  # Update registre consentements to include PRM-level revocation status

  1. Changes
    - Update get_registre_consentements() to check both authorization AND individual PRM status
    - Display 'REVOKED' if either the authorization OR the individual PRM is revoked
    - Include revocation reason in the export

  2. Logic
    - Priority: PRM.statut takes precedence over authorization status
    - If PRM.statut = 'REVOKED' → show 'REVOKED' even if authorization is ACTIVE
    - If authorization is REVOKED → show 'REVOKED' for all PRMs
*/
...
BEGIN
  ...
  SELECT 
    ...
    -- Get revocation reason from consent_events
    COALESCE(
      (SELECT ce.metadata->>'revocation_reason' 
       FROM consent_events ce 
       WHERE ce.prm_id = p.id 
         AND ce.event_type IN ('PRM_REVOKED', 'CONSENT_REVOKED')
