...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { contactId, message, expirationDays = 30, appUrl }: AuthorizationRequest = await req.json();
    ...
    const authorizationUrl = `${baseUrl}/upload/${token}`;
...
<body>
  <div class="container">
    <div class="header">
      <h1 style="margin: 0;">Demande d'autorisation de communication des données</h1>
    </div>
    <div class="content">
      <p>Bonjour ${contact.contact1_prenom} ${contact.contact1_nom},</p>

      <p>Nous avons besoin de recueillir votre autorisation pour effectuer votre simulation énergétique. Cette autorisation nous permettra de récupérer vos données de consommation auprès de votre gestionnaire de réseau (Enedis).</p>

      ${message ? `<div class="info-box"><strong>Message personnalisé :</strong><br>${message}</div>` : ''}

      <div style="text-align: center;">
        <a href="${authorizationUrl}" class="button">✍️ Compléter et valider le formulaire</a>
      </div>

      <div class="info-box">
        <strong>Informations importantes :</strong>
        <ul style="margin: 10px 0; padding-left: 20px;">
          <li>Vous devrez fournir vos numéros de PRM (Point de Référence et de Mesure)</li>
          <li>Vos données sont sécurisées et confidentielles</li>
          <li>Cette autorisation est valable pour 36 mois</li>
      ...
      <a href="${authorizationUrl}" style="color: #2563eb; word-break: break-all;">${authorizationUrl}</a></p>
    </div>
    <div class="footer">
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Demande d'autorisation de communication des données - ${contact.entreprise}`,
        html: emailHtml,
      }),
    });
    ...
    console.error("Error in send-authorization-request:", error);
    return new Response(
      JSON.stringify({
